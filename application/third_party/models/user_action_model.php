...
class User_action_model extends CI_Model
{
    ...
    public function get_all_buying_won_details($where= array(), $view,$session_id,$limit,$offset,$shortlist_item,$order_by){ 
        $query = $this->db->query("SELECT * FROM `lists`WHERE buyer_id=$session_id AND status='sold'  AND is_admin_deleted='0' AND is_deleted='0' ORDER BY lists.$shortlist_item $order_by LIMIT $limit  OFFSET $offset");   
         
       //echo $this->db->last_query();
       return $query->result(); 
    }
    public function get_all_buying_won_pagination($where= array(), $view,$session_id,$limit,$offset){ 
        $query = $this->db->query("SELECT COUNT(DISTINCT id) AS total FROM `lists` WHERE buyer_id=$session_id AND is_deleted='0' AND is_admin_deleted='0' AND  status='sold'");   
    ...
    public function get_all_selling_details($where= array(), $view,$session_id,$limit,$offset,$shortlist_item,$order_by){ 
        $query = $this->db->query("SELECT * FROM lists  WHERE  is_deleted ='0' AND is_admin_deleted='0' AND user_id=$session_id  GROUP BY id ORDER BY $shortlist_item $order_by LIMIT $limit  OFFSET $offset");   
         
        //echo $this->db->last_query();
        return $query->result(); 
    }
    public function get_all_selling_pagination($where= array(), $view,$session_id,$limit,$offset){ 
        $query = $this->db->query("SELECT count(id) as total FROM lists WHERE is_deleted ='0' AND is_admin_deleted='0' AND user_id=$session_id ");   
         
        //echo $this->db->last_query();
        return $query->row(); 
    }
    public function get_schedule_list($where= array(), $view,$session_id,$limit,$offset,$shortlist_item,$order_by){    
        $date = date("Y-m-d H:i:s");
        $query = $this->db->query("SELECT * FROM lists  WHERE  is_deleted ='0' AND is_admin_deleted='0' AND is_sold='0' AND  user_id=$session_id  AND end_auction > '$date'  GROUP BY id ORDER BY $shortlist_item $order_by LIMIT $limit  OFFSET $offset");  
         
       //echo $this->db->last_query();
       return $query->result(); 
    }
    public function get_schedule_list_pagination($where= array(), $view,$session_id,$limit,$offset){ 
        $date = date("Y-m-d H:i:s");
        $query = $this->db->query("SELECT count(id) as total FROM lists  WHERE  is_deleted ='0' AND is_admin_deleted='0' AND is_sold='0' AND  user_id=$session_id  AND end_auction > '$date'");   
         
       //echo $this->db->last_query();
       return $query->row(); 
    }

    public function get_sold_list($where= array(), $view,$session_id,$limit,$offset,$shortlist_item,$order_by){ 
        $query = $this->db->query("SELECT * FROM lists  WHERE  is_sold ='1' AND is_admin_deleted='0' AND is_deleted ='0' AND status ='sold' AND is_sold='1' AND user_id=$session_id  GROUP BY id ORDER BY $shortlist_item $order_by LIMIT $limit  OFFSET $offset");   
         
       //echo $this->db->last_query();
       return $query->result(); 
    }
    public function get_sold_list_pagination($where= array(), $view,$session_id,$limit,$offset){ 
        $query = $this->db->query("SELECT count(id) as total FROM lists WHERE is_sold ='1' AND is_admin_deleted='0' AND is_deleted ='0' AND status ='sold'  AND is_sold='1' AND user_id=$session_id ");   
         
       //echo $this->db->last_query();
       return $query->row(); 
    }

    public function get_unsold_list($where= array(), $view,$session_id,$limit,$offset,$shortlist_item,$order_by){ 
        $date = date("Y-m-d H:i:s");
        $query = $this->db->query("SELECT * FROM lists  WHERE  is_sold ='0'  AND is_deleted ='0' AND is_admin_deleted='0' AND  user_id=$session_id  AND end_auction < '$date' GROUP BY id ORDER BY $shortlist_item $order_by LIMIT $limit  OFFSET $offset");   
         
       //echo $this->db->last_query();
       return $query->result(); 
    }
    public function get_unsold_list_pagination($where= array(), $view,$session_id,$limit,$offset){ 
        $date = date("Y-m-d H:i:s");
        $query = $this->db->query("SELECT count(id) as total FROM lists WHERE is_sold ='0' AND is_deleted ='0' AND is_admin_deleted='0' AND user_id=$session_id AND end_auction < '$date' ");   
    ...
    public function get_all_video($where= array(), $view,$session_id,$limit,$offset){ 
        $query = $this->db->query("SELECT list_attachments.*,lists.slug,lists.title,lists.buy_now_price,lists.status,lists.is_sold,lists.end_auction FROM list_attachments INNER JOIN lists ON list_attachments.list_id=lists.id WHERE  type ='vimeo_id'   ORDER BY id DESC LIMIT $limit  OFFSET $offset");            
        //echo $this->db->last_query();
       
        return $query->result(); 
    }
    public function get_all_video_pagination($where= array(), $view,$session_id,$limit,$offset){ 
        $query = $this->db->query("SELECT COUNT(list_attachments.id) as total  FROM list_attachments INNER JOIN lists ON list_attachments.list_id=lists.id WHERE  type ='vimeo_id'  ");            
        //echo $this->db->last_query();
