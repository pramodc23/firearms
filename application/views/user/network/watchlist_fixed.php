    ...
    <tr>
      <!--   <td>
       ...
       ?></span></span>
        ...
        ?>
          <p id="auctionwat_<?php echo $i; ?>" class="<?php echo $a_cls;?>"></p>
        </td>
...
//var x = setInterval(function() {
    ...
    if(days>0 && hours>0){
      document.getElementById("auctionwat_"+d_id).innerHTML = days + "d " + hours + 'h +';
    }else if(days<=0 && hours>0 && minutes>0){
      document.getElementById("auctionwat_"+d_id).innerHTML = hours + "h "+ minutes + 'm +';
    }else if(days<=0 && hours<=0 && minutes>0 && seconds>0){
      document.getElementById("auctionwat_"+d_id).innerHTML = minutes + "m "+ seconds + 's +';
    }else if(days<=0 && hours<=0 && minutes<=0 && seconds>0){
      document.getElementById("auctionwat_"+d_id).innerHTML = seconds + "s";
    }else if(days<=0 && hours<=0 && minutes<=0 && seconds<=0){
      clearInterval(x);
      document.getElementById("auctionwat_"+d_id).innerHTML = "EXPIRED";
    }
  ...
  $foundnum =$count;
        ...
        {   
            ...
            if (!($start<=0)){
             ...
             echo " <li><a href='javascript:watch_list_item_fixed($prev)'>Prev</a> </li>";    
            }
            //pages 
            ...
            {               
                ...
                for ($counter = 1; $counter <= $max_pages; $counter++)
                {
                    if ($i == $start)
                    {
                        echo " <li  class='active'><a href='javascript:watch_list_item_fixed($i)'><b>$counter</b></a></li> ";
                    }
                    else
                    {
                        echo " <li><a href='javascript:watch_list_item_fixed($i)'>$counter</a></li> ";
                    }  
                    $i = $i + $per_page;                 
                }
            }
            elseif($max_pages > 10 + ($adjacents * 2))    //enough pages to hide some
            {
                ...
                if(($start/$per_page) < 1 + ($adjacents * 2))        
                {
                    ...
                    for ($counter = 1; $counter < 10 + ($adjacents * 2); $counter++)
                    {
                        if ($i == $start)
                        {
                            echo " <li  class='active'><a href='javascript:watch_list_item_fixed($i)'>$counter</a></li> ";
                        }
                        else
                        {
                            echo " <li><a href='javascript:watch_list_item_fixed($i)'>$counter</a></li> ";
            ...
            if (!($start >=$foundnum-$per_page)){
              echo " <li><a href='javascript:watch_list_item_fixed($next)'>Next</a></li> ";
            }
