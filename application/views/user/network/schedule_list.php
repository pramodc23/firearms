<div class="table-section-08">
  <div class="side-by-side">
    <div class="row">
      <div class="col-md-8">
        <p>Show Items- Schedule  &nbsp;&nbsp;All (<?php echo count($network_info); ?>)  <!-- &nbsp;&nbsp;Active (1)  &nbsp;&nbsp;Ended --></p>
      </div>
...
<script type="text/javascript">
  $('#selling_checked_all').click(function() {
    if ($(this).is(':checked')) { 
      ...
      // $('#delete_all').show();
      $('#relist_schedule_all').show();
    }else{
      $(".selling_all_check").prop("checked", false);   
      // $('#delete_all').hide();  
      $('#relist_schedule_all').hide(); 
    }  
  });
</script>
<script type="text/javascript">
    ...
    <thead class="text-muted">
      <tr>
        <th scope="col" class="text-left" >
          <input id="selling_checked_all" type="checkbox" > 
          </th>
      ...
      <tr>
        ...
        <td> <span class="bid_font">
            <?php 
             ...
             //echo   $end_date = date('M d, Y H:i:s',strtotime($value->end_auction));
            ?>
          </span>
        </td>
       <td class="text-right">     
          ...
          <select class="filter_select" onchange="selling_item(this.value,'<?php echo $list_id; ?>','<?php echo $slug;?>');" style="-webkit-appearance: none;margin-top: 13px;">
            ...
            <?php } ?>
            <?php if($value->type==1){ echo '<option value="user_bids">All Bids</option>
          '; } ?>
          <?php
            if($value->type==1)
            {
           ...
           ?>
            <?php  if ($bid_detail->total_bid > 0 && $value->type==1) {
              ?>
              <?php
...
<button id="delete_all" onclick="delete_all_list();" style="display: none;margin-left:20px;" >Delete All</button>
<button id="relist_schedule_all" onclick="relist_schedule_all();" style="display: none;margin-left:20px;background-color: #ef6c03;padding: 8px 15px;color: white;font-weight: 600;"  >Relist All</button>
  ...
  $foundnum =$count;
        ...
        {   
            ...
            if (!($start<=0)){
             ...
             echo " <li><a href='javascript:selling_schedule($prev)'>Prev</a> </li>";    
            }
            //pages 
            ...
            {               
                ...
                for ($counter = 1; $counter <= $max_pages; $counter++)
                {
                    if ($i == $start)
                    {
                        echo " <li  class='active'><a href='javascript:selling_schedule($i)'><b>$counter</b></a></li> ";
                    }
                    else
                    {
                        echo " <li><a href='javascript:selling_schedule($i)'>$counter</a></li> ";
                    }  
                    $i = $i + $per_page;                 
                }
            }
            elseif($max_pages > 10 + ($adjacents * 2))    //enough pages to hide some
            {
                ...
                if(($start/$per_page) < 1 + ($adjacents * 2))        
                {
                    ...
                    for ($counter = 1; $counter < 10 + ($adjacents * 2); $counter++)
                    {
                        if ($i == $start)
                        {
                            echo " <li  class='active'><a href='javascript:selling_schedule($i)'>$counter</a></li> ";
                        }
                        else
                        {
                            echo " <li><a href='javascript:selling_schedule($i)'>$counter</a></li> ";
            ...
            if (!($start >=$foundnum-$per_page)){
              echo " <li><a href='javascript:selling_schedule($next)'>Next</a></li> ";
            }
