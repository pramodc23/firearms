...
public function ask_question_mail_seller($to,$from,$subject,$question,$seller_name,$buyer_name)
      {
        ...
        $config['mailtype'] = 'html';
        $config['wordwrap'] = TRUE;
        $body=$this->load->view('front/ask_question_mail',$data,TRUE);
      

        $this->email->initialize($config);  
        $this->email->from($from, 'Firearms Network');
        $this->email->to($to); 
        $this->email->subject($subject);
        $this->email->message($body);
        $this->email->send();
